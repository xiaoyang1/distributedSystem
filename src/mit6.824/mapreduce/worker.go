...
type Worker struct {
	...
	l           net.Listener
	parallelism *Parallelism  // 因为这里用的是外部同一个 Parallelism， 所以可以监控
}
...
func (wk *Worker) DoTask(arg *DoTaskArgs, _ *struct{}) error {
	...
	if wk.parallelism != nil {
		...
		//fmt.Printf("wk.parallelism.now : %d, wk.parallelism.max : %d", wk.parallelism.now, wk.parallelism.max)
	}
...
func RunWorker(MasterAddress string, me string,
	...
	rpcs := rpc.NewServer()
	_ = rpcs.Register(wk)
	_ = os.Remove(me) // only needed for "unix"
